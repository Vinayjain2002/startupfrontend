...
export const Login = () => {
  ...
  const [loginUser] = useLazyQuery(LOGIN_USER, {
    onCompleted: (res) => {
      // Make sure the response has the login data and accessToken
      if (res?.login?.accessToken) {
        localStorage.setItem("UserToken", res.login.accessToken);
        toast.success("Login successful!");  // Custom success message
        setIsLoading(false);
        pageRoute("/home");
      } else {
        setIsLoading(false);
  ...
  // validating the user on the initial rendering of the page
  useEffect(() => {
    const isValid = async () => {
      // need to define the route for the validation of the user
      const data = "";
      if (data?.accessToken) {
